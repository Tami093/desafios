...
public class IdwallFormatter extends StringFormatter {
	...
	private static void formatacaoTexto(String texto) {
		...
		while (auxSairWhile != (texto.length() + CONSTANTE_1)) {
			if (auxTamanhoAteVaiString == texto.length()) {
				Texto_Formatado_Parte1 += retiraEspacoComecoFimDaFrase(
						texto.substring(auxInicioDaString, auxTamanhoAteVaiString));
				...
				if (texto.substring((auxTamanhoAteVaiString - CONSTANTE_1), auxTamanhoAteVaiString).equals(" ") || texto
						.substring(auxTamanhoAteVaiString, (auxTamanhoAteVaiString + CONSTANTE_1)).equals(" ")) {
					Texto_Formatado_Parte1 += retiraEspacoComecoFimDaFrase(
							texto.substring(auxInicioDaString, auxTamanhoAteVaiString));
					Texto_Formatado_Parte2 += (adicionaEspacosAMais(
							retiraEspacoComecoFimDaFrase(texto.substring(auxInicioDaString, auxTamanhoAteVaiString))));
	...
	/**
	 * Função que retorna o tamanho a ser lido da string, sendo o cursor do
	 * final lido + o tamanho linha, ou o final do cursor mais oque é preciso
	 * ler.
	 * 
	 * @param texto
	 *            texto a ser lido
	 * @param auxFinalTamnhoString
	 *            altera este parametro para o novo final da string
	 ...
	 * 
	 * @param tamanhoFim
	 *            tamanho aonde esta localizado o char da palava quebrando-a
	 * @param texto
	 *            Texto a ser lido
